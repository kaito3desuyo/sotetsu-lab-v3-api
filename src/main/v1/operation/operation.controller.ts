...
export class OperationController {
    ...
    async searchOperationsCurrentPosition(
        @Query()
        query: {
            calendar_id: string;
        },
        ...
        const operations = await this.operationService
            ...
            .andWhere('operations.operation_number != :number', {
                number: '100',
            })
        ...
        const currentPosition = operations.map((operation) => {
            ...
            if (
                ...
                moment(
                    ...
                    .add(
                        operation.trip_operation_lists[0].start_time
                            .departure_days - 1,
                        'days',
                    )
            ) {
                ...
                (trip, index, array) => {
                    ...
                    return (
                        moment(array[index].end_time.arrival_time, 'HH:mm:ss')
                            .subtract(now.hour() < 4 ? 1 : 0, 'days')
                            .add(
                                array[index].end_time.arrival_days - 1,
                                'days',
                            ) <= now &&
                        now <
                            moment(
                                array[index + 1].start_time.departure_time,
                                'HH:mm:ss',
                            )
                                .subtract(now.hour() < 4 ? 1 : 0, 'days')
                                .add(
                                    array[index + 1].start_time.departure_days -
                                        1,
                                    'days',
                                )
                    );
            ...
            const nMinus1ToNowToNDep = find(
                ...
                (trip, index, array) => {
                    ...
                    return (
                        moment(
                            array[index - 1].end_time.arrival_time,
                            'HH:mm:ss',
                        )
                            .subtract(now.hour() < 4 ? 1 : 0, 'days')
                            .add(
                                array[index - 1].end_time.arrival_days - 1,
                                'days',
                            ) <= now &&
                        now <
                            moment(
                                array[index].start_time.departure_time,
                                'HH:mm:ss',
                            )
                                .subtract(now.hour() < 4 ? 1 : 0, 'days')
                                .add(
                                    array[index].start_time.departure_days - 1,
                                    'days',
                                )
                    );
                },
            ...
            const currentRunning = find(
                ...
                (tripOperationList, index, array) => {
                    return (
                        moment(
                            tripOperationList.start_time.departure_time,
                            'HH:mm:ss',
                        )
                            .subtract(now.hour() < 4 ? 1 : 0, 'days')
                            .add(
                                tripOperationList.start_time.departure_days - 1,
                                'days',
                            ) <= now &&
                        now <
                            moment(
                                tripOperationList.end_time.arrival_time,
                                'HH:mm:ss',
                            )
                                .subtract(now.hour() < 4 ? 1 : 0, 'days')
                                .add(
                                    tripOperationList.end_time.arrival_days - 1,
                                    'days',
                                )
                    );
                },
    ...
    @Get('/trips')
    async getOperationsTrips(
        @Query() query: { calendar_id: string },
    ): Promise<{
    ...
    async getOperationsByCalendarIdSightings(
        ...
        return operations.map((operation) => {
            return {
                ...operation,
                operation_sightings: operationSightings.filter(
                    (sighting, index) => {
                        if (index > 10) {
                            return false;
                        }
...
const searchCalendarId = (
    ...
    return qb.andWhere(`${tableName}.calendar_id = :calendarId`, {
        calendarId,
    });
};
