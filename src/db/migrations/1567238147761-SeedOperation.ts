    ...
    public async up(queryRunner: QueryRunner): Promise<any> {
        ...
        operationsSeed.forEach(seed => {
            const temp = seed.operations.map(operation => {
                return {
                    calender_id: find(calenders, (calender) => calender.calender_name === seed.calender_name && calender.start_date === seed.start_date).id,
                    operation_number: operation.operation_number
                }
            })

            temp.forEach(temp => operations.push(temp))
        })
...
const operationsSeed: {
    ...
    start_date: string;
    operations: {operation_number: string}[]
}[] = [
    {
        calender_name: '平日ダイヤ',
