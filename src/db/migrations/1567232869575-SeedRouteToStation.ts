...
export class SeedRouteToStation1567232869575 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<any> {
        ...
        const routeStations = routeStationListsSeed.map(routeStation => {
            routeStation['route_id'] = find(routes, (route) => route.route_name === routeStation.route_name).id
            routeStation['station_id'] = find(stations, (station) => station.station_name === routeStation.station_name).id
...
const routeStationListsSeed: {route_name: string, station_name: string, station_sequence: number, station_numbering: string}[] = [
    {
        route_name: '本線',
        station_name: '横浜',
        station_sequence: 1,
