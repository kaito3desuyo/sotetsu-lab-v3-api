  ...
  public async up(queryRunner: QueryRunner): Promise<any> {
    ...
    const operatingSystems = operatingSystemsSeed.map(operatingSystem => {
      ...
      const temp = {
        ...
        service_id: find(
          services,
          service => service.service_name === operatingSystem.service_name,
        ).id,
        route_id: route.id,
        start_route_station_list_id: find(
          route.route_station_lists,
          rs => rs.station.station_name === operatingSystem.start_station_name,
        ).id,
        end_route_station_list_id: find(
          route.route_station_lists,
          rs => rs.station.station_name === operatingSystem.end_station_name,
        ).id,
      };
